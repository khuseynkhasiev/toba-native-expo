...
export default function SeriesFourNewTest({navigation}) {
    ...
    const [isActiveBtnNavigation, setIsActiveBtnNavigation] = useState(false);
    ...
    async function moveToNextScene() {
        if (
            ...
            setIsActiveBtnNavigation(false);
        }
        if (
            posMillis >= sceneTwoPause &&
            posMillis <= sceneTwoPause + sceneStopInterval
