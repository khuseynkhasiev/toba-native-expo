...
*/
    ...
    function handleClickReadSeries() {
        ...
        if (item.id === 1) {
            ...
            navigation.navigate("SeriesOneNew", {
        ...
        if (item.id === 4) {
            //navigation.navigate('SeriesOnePage');
            navigation.navigate("TestVideoOne", {
                isActiveBackgroundMusic: isActive,
            });
        }
        if (item.id === 5) {
    ...
    return (
        <TouchableOpacity
            style={[
                styles.series__container,
                isActiveSeries &&
                    styles.series__container_active /* { width: itemWidth, height: sliderHeight }*/,
            ]}
            onPress={() => handleClickSeriesCard()}
        >
            <ImageBackground
                style={[
                    styles.series__titleBackground,
                    isActiveSeries && styles.series__titleBackground_active,
                ]}
            ...
            >
                <Image
                    style={[
                        styles.series__seriesImg,
                        isActiveSeries && styles.series__seriesImg_active,
                    ]}
                    source={item.img}
                />
                <View style={styles.series__info}>
                    <Text
                        style={[
                            styles.series__nameSeriesNumber,
                            isActiveSeries &&
                                styles.series__nameSeriesNumber_active,
                        ]}
                    >
                        {item.title}
                    ...
                    {isActiveSeries ? (
                        <Text style={styles.series__subtitleMax}>
                            {item.subtitleMax}
                        </Text>
                    ) : (
                        <Text style={styles.series__subtitle}>
                            {item.subtitle}
                        </Text>
                    )}
                </View>
                {isActiveSeries && (
                    <TouchableOpacity
                        style={styles.series__readBtn}
                        onPress={() => handleClickReadSeries()}
                    >
                        <LottieView
