...
export default function ProfileEditPassword({ navigation }) {
    ...
    const [newPasswordErrorInputText, setNewPasswordErrorInputText] =
        useState("");

    const [newPasswordRepeat, setNewPasswordRepeat] = useState("");
    const [newPasswordRepeatIsError, setNewPasswordRepeatIsError] =
        useState(false);
    const [
        newPasswordRepeatErrorInputText,
        setNewPasswordRepeatErrorInputText,
    ] = useState("");
    ...
    function handleBlurInputNewPassword() {
        ...
        if (newPassword.length < 1) {
            ...
            if (hasUpperCaseAndLowerCase(newPassword)) {
                ...
                setNewPasswordIsError(true);
                setNewPasswordErrorInputText(
                    "Не меньше 8 символов и символ верхнего и нижнего регистра"
                );
                setOpenNewPassword(false);
            }
        }
                            ...
                            <View style={styles.input__container}>
                                <TextInput
                                    style={[
                                        styles.input__password,
                                        {
                                            color: newPasswordRepeatIsError
                                                ? "red"
                                                : "#FFF",
                                        },
                                    ]}
                                    ...
                                    onBlur={() =>
                                        handleBlurInputRepeatNewPassword()
                                    }
                                />
                                <TouchableOpacity
                                    style={styles.passwordIconSvgButton}
                                    onPress={() =>
                                        setOpenNewPasswordRepeat(
                                            !openNewPasswordRepeat
                                        )
                                    }
                                >
